...
const socialHumanReviewEnv = "GIZCLAW_E2E_SOCIAL_HUMAN_REVIEW"
...
func TestServerSocialRPCHumanReview(t *testing.T) {
	if strings.TrimSpace(os.Getenv(socialHumanReviewEnv)) == "" {
		t.Skipf("set %s=1 to run the social human-review audio case", socialHumanReviewEnv)
	}
	...
	group := mustRunCLIJSON[rpcapi.FriendGroupCreateResponse](t, h, "connect", "friend-group", "create", "human-review", "--context", "peer-a")
	if stringValue(group.WorkspaceName) == "" {
		t.Fatalf("friend_group.create workspace_name is empty: %#v", group)
	}
	mustRunCLIJSON[rpcapi.FriendGroupMemberAddResponse](t, h, "connect", "friend-group", "members", "add", stringValue(group.Id), peerB, "--role", "member", "--context", "peer-a")
	mustRunCLIJSON[rpcapi.FriendGroupMemberAddResponse](t, h, "connect", "friend-group", "members", "add", stringValue(group.Id), peerC, "--role", "member", "--context", "peer-a")
...
func newSocialHumanReviewHarness(t *testing.T) *clitest.Harness {
	...
	applySocialHumanReviewSharedResources(t, h)
	for _, peer := range []string{"peer-a", "peer-b", "peer-c"} {
		...
		applySocialHumanReviewPeerConfig(t, h, peer)
	}
	return h
}
...
func requireSocialHumanReviewProviderEnv(t *testing.T) {
	...
	if len(missing) > 0 {
		t.Skipf("set %s and %s provider env to run social human-review audio", socialHumanReviewEnv, strings.Join(missing, ", "))
	}
	if strings.TrimSpace(os.Getenv("GIZCLAW_E2E_VOICE_RESOURCE")) == "" {
		t.Setenv("GIZCLAW_E2E_VOICE_RESOURCE", "volc-tenant:"+socialHumanReviewVolcTenant()+":"+strings.TrimSpace(os.Getenv("GIZCLAW_E2E_VOLC_VOICE_ID")))
	}
}
...
func applySocialHumanReviewSharedResources(t *testing.T, h *clitest.Harness) {
	t.Helper()

	for _, name := range []string{
		"02-volc-credential.json",
		"03-volc-tenant.json",
		"11-tts-model.json",
		"12-asr-model.json",
		"30-acl-view.json",
		"30-client-acl-role.json",
		"40-workflow-chatroom.json",
		"51-view-acl-credential-volc.json",
		"53-view-acl-model-tts.json",
		"54-view-acl-model-asr.json",
		"57-view-acl-workflow.json",
		"58-view-acl-workspace.json",
	} {
		h.RunCLI("admin", "apply", "-f", socialHumanReviewResourcePath(h, name), "--context", "admin-a").MustSucceed(t)
	}
	h.RunCLI("admin", "volc-tenants", "--context", "admin-a", "sync-voices", socialHumanReviewVolcTenant()).MustSucceed(t)
	h.RunCLI("admin", "apply", "-f", socialHumanReviewResourcePath(h, "56-view-acl-voice.json"), "--context", "admin-a").MustSucceed(t)
}

func applySocialHumanReviewPeerConfig(t *testing.T, h *clitest.Harness, peer string) {
	t.Helper()

	path := filepath.Join(h.SandboxDir, peer+"-peer-config.json")
...
`, h.ContextPublicKey(peer), socialHumanReviewACLView())
	if err := os.WriteFile(path, data, 0o644); err != nil {
		t.Fatalf("write %s social human-review peer config: %v", peer, err)
	}
	h.RunCLI("admin", "apply", "-f", path, "--context", "admin-a").MustSucceed(t)
}

func socialHumanReviewResourcePath(h *clitest.Harness, name string) string {
	return filepath.Join(h.RepoRoot, "test", "gizclaw-e2e", "setup", "resources", name)
}

func socialHumanReviewVolcTenant() string {
	if value := strings.TrimSpace(os.Getenv("GIZCLAW_E2E_VOLC_TENANT")); value != "" {
		return value
	}
	return "e2e-volc-tenant"
}

func socialHumanReviewACLView() string {
	if value := strings.TrimSpace(os.Getenv("GIZCLAW_E2E_ACL_VIEW")); value != "" {
		return value
	}
	return "e2e-client"
}

func socialHumanReviewTTSModel() string {
	if value := strings.TrimSpace(os.Getenv("GIZCLAW_E2E_TTS_MODEL")); value != "" {
		return value
	}
	return "e2e-tts"
}

func socialHumanReviewVoiceResource() string {
	return strings.TrimSpace(os.Getenv("GIZCLAW_E2E_VOICE_RESOURCE"))
}
