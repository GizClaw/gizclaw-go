...
import (
	...
	itest "github.com/giztoy/giztoy-go/cmd/internal/integration/testutil"
...
func NewHarness(t testing.TB, story string) *Harness {
	...
	h := &Harness{
		t:               t,
		RepoRoot:        repoRoot,
		StoryDir:        filepath.Join(repoRoot, "cmd", "internal", "integration", "cli", story),
...
func (h *Harness) ContextPublicKey(name string) string {
	...
	keyPair, err := identity.Load(filepath.Join(h.contextRoot(), name, "identity.key"))
	if err != nil {
...
func (h *Harness) waitForServerIdentity() {
	...
	if err := itest.WaitUntil(itest.ReadyTimeout, func() error {
		...
		keyPair, err := identity.Load(identityPath)
		if err != nil {
			return err
		}
...
func (h *Harness) waitForServerReady() {
	...
	if err := itest.WaitUntil(itest.ReadyTimeout, func() error {
		...
		client, err := gizclaw.Dial(keyPair, h.ServerAddr, serverPublicKey)
		if err != nil {
			return err
		}
		defer client.Close()
...
func mustRepoRoot(t testing.TB) string {
	...
	if !ok {
		t.Fatal("resolve cmd/internal/integration/cli harness path")
	}
