...
var (
	...
	modelsByKindRoot     = kv.Key{"by-kind"}
...
func (s *Server) ListModels(ctx context.Context, request adminservice.ListModelsRequestObject) (adminservice.ListModelsResponseObject, error) {
	...
	if request.Params.Kind != nil {
		kind := strings.TrimSpace(string(*request.Params.Kind))
		if kind != "" {
			filters.kind = &kind
		}
	}
...
func normalizeModelUpsert(in adminservice.ModelUpsert, expectedID string) (apitypes.Model, error) {
	...
	if in.ProviderData != nil {
		model.ProviderData = cloneMap(in.ProviderData)
	}
	return model, nil
}
...
func listModelsPage(ctx context.Context, store kv.Store, filters modelFilters, cursor string, limit int) ([]apitypes.Model, bool, *string, error) {
	...
	case filters.kind != nil:
		prefix = modelByKindPrefix(*filters.kind)
...
func matchesModelFilters(model apitypes.Model, filters modelFilters) bool {
	if filters.kind != nil && string(model.Kind) != *filters.kind {
		return false
	}
...
func writeModel(ctx context.Context, store kv.Store, model apitypes.Model, previous *apitypes.Model) error {
	...
	entries := []kv.Entry{
		...
		{Key: modelByKindKey(string(model.Kind), string(model.Id)), Value: []byte{}},
...
func staleModelIndexKeys(previous, next apitypes.Model) []kv.Key {
	...
	if previous.Kind != next.Kind {
		keys = append(keys, modelByKindKey(string(previous.Kind), string(previous.Id)))
	}
...
func deleteModel(ctx context.Context, store kv.Store, model apitypes.Model) error {
	keys := []kv.Key{
		...
		modelByKindKey(string(model.Kind), string(model.Id)),
...
func modelByKindPrefix(kind string) kv.Key {
	return append(append(kv.Key{}, modelsByKindRoot...), escapeStoreSegment(kind))
}

func modelByKindKey(kind, id string) kv.Key {
	return append(modelByKindPrefix(kind), escapeStoreSegment(id))
}
