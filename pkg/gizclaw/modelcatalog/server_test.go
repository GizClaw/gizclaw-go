...
func TestServerModelCRUDListFiltersAndIndexes(t *testing.T) {
	...
	first := modelUpsert("qwen-flash", apitypes.ModelKindLlm, "dashscope", "cn")
	first.Name = stringPtr("Qwen Flash")
	first.ProviderData = &map[string]interface{}{
		"openai-compatible": map[string]interface{}{"base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1"},
	}
	second := modelUpsert("speech", apitypes.ModelKindTts, "openai", "global")
	...
	kind := adminservice.ModelKind(apitypes.ModelKindLlm)
	filteredResp, err := srv.ListModels(ctx, adminservice.ListModelsRequestObject{
		Params: adminservice.ListModelsParams{Kind: &kind},
	})
	if err != nil {
		t.Fatalf("ListModels(kind) error = %v", err)
	}
	filtered := requireModelList(t, filteredResp)
	if len(filtered.Items) != 1 || filtered.Items[0].Id != "qwen-flash" {
		t.Fatalf("ListModels(kind) items = %#v", filtered.Items)
	}

	providerKind := adminservice.ModelProviderKind("openai")
	providerName := adminservice.ModelProviderName("global")
	...
	updated := first
	updated.Kind = apitypes.ModelKindAsr
	updated.Provider = apitypes.ModelProvider{Kind: "openai", Name: "global"}
	...
	oldFilterResp, err := srv.ListModels(ctx, adminservice.ListModelsRequestObject{
		Params: adminservice.ListModelsParams{Kind: &kind},
	})
	if err != nil {
		t.Fatalf("ListModels(old kind) error = %v", err)
	}
	if oldFiltered := requireModelList(t, oldFilterResp); len(oldFiltered.Items) != 0 {
		t.Fatalf("ListModels(old kind) = %#v", oldFiltered.Items)
	}
	...
	if got, ok := getResp.(adminservice.GetModel200JSONResponse); !ok || got.Kind != apitypes.ModelKindAsr {
...
func TestServerListModelsPagination(t *testing.T) {
	...
	for _, id := range []string{"a", "b", "c"} {
		upsert := modelUpsert(id, apitypes.ModelKindLlm, "openai", "main")
...
func TestServerRejectsInvalidAndSyncModelWrites(t *testing.T) {
	...
	invalid := modelUpsert("bad", apitypes.ModelKind("image"), "openai", "main")
	if resp, err := srv.CreateModel(ctx, adminservice.CreateModelRequestObject{Body: &invalid}); err != nil {
		t.Fatalf("CreateModel(invalid) error = %v", err)
	} else if _, ok := resp.(adminservice.CreateModel400JSONResponse); !ok {
		t.Fatalf("CreateModel(invalid) response = %#v", resp)
	}
	...
	manual := modelUpsert("synced", apitypes.ModelKindLlm, "openai", "main")
...
func TestServerModelValidationAndErrorResponses(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "missing id", body: adminservice.ModelUpsert{Kind: apitypes.ModelKindLlm, Source: apitypes.ModelSourceManual, Provider: apitypes.ModelProvider{Kind: "openai", Name: "main"}}},
		{name: "sync source", body: adminservice.ModelUpsert{Id: "sync", Kind: apitypes.ModelKindLlm, Source: apitypes.ModelSourceSync, Provider: apitypes.ModelProvider{Kind: "openai", Name: "main"}}},
		...
		{name: "missing provider name", body: adminservice.ModelUpsert{Id: "provider", Kind: apitypes.ModelKindLlm, Source: apitypes.ModelSourceManual, Provider: apitypes.ModelProvider{Kind: "openai"}}},
	} {
	...
	base := modelUpsert("manual", apitypes.ModelKindLlm, "openai", "main")
...
func TestServerListModelsSourceFilterAndSyncedTimePreserved(t *testing.T) {
	...
	previous := apitypes.Model{
		Id:       "sync-preserved",
		Kind:     apitypes.ModelKindLlm,
		Provider: apitypes.ModelProvider{Kind: "openai", Name: "main"},
	...
	update := modelUpsert("sync-preserved", apitypes.ModelKindTts, "openai", "main")
...
func modelUpsert(id string, kind apitypes.ModelKind, providerKind, providerName string) adminservice.ModelUpsert {
	return adminservice.ModelUpsert{
		Id:     apitypes.ModelID(id),
		Kind:   kind,
		Source: apitypes.ModelSourceManual,
		Provider: apitypes.ModelProvider{
			Kind: apitypes.ModelProviderKind(providerKind),
			Name: apitypes.ModelProviderName(providerName),
		},
	}
}
