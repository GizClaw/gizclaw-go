...
func (s *Server) adminHandler(publicKey string) http.Handler {
	...
	fwHandler := s.firmware.AdminHandler()
	fwPrefix := s.firmware.PathPrefix()
	mux.HandleFunc(fwPrefix, s.withCaller(publicKey, s.requireAdmin(fwHandler.ServeHTTP)))
	mux.HandleFunc(fwPrefix+"/", s.withCaller(publicKey, s.requireAdmin(fwHandler.ServeHTTP)))
	return mux
}
...
func (s *Server) handleAdminGearByPath(w http.ResponseWriter, r *http.Request) {
	...
	case strings.HasPrefix(path, "sn/") && r.Method == http.MethodGet:
		...
		if err != nil {
			httputil.WriteError(w, http.StatusNotFound, "GEAR_SN_NOT_FOUND", err.Error())
			return
		}
		httputil.WriteJSON(w, http.StatusOK, map[string]string{"public_key": gear.PublicKey})
	case strings.HasPrefix(path, "imei/") && r.Method == http.MethodGet:
		parts, err := decodeEscapedSegments(strings.TrimPrefix(path, "imei/"), 2)
		if err != nil {
			httputil.WriteError(w, http.StatusBadRequest, "INVALID_PARAMS", "want /gears/imei/{tac}/{serial}")
			return
		}
		gear, err := s.gears.ResolveByIMEI(r.Context(), parts[0], parts[1])
		if err != nil {
			httputil.WriteError(w, http.StatusNotFound, "GEAR_IMEI_NOT_FOUND", err.Error())
			return
		}
		httputil.WriteJSON(w, http.StatusOK, map[string]string{"public_key": gear.PublicKey})
	case strings.HasPrefix(path, "label/") && r.Method == http.MethodGet:
		...
		if err != nil {
			httputil.WriteError(w, http.StatusBadRequest, "INVALID_PARAMS", "want /gears/label/{key}/{value}")
			return
		}
	...
	case strings.HasPrefix(path, "certification/") && r.Method == http.MethodGet:
		...
		if err != nil {
			httputil.WriteError(w, http.StatusBadRequest, "INVALID_PARAMS", "want /gears/certification/{type}/{authority}/{id}")
			return
		}
	...
	case strings.HasPrefix(path, "firmware/") && r.Method == http.MethodGet:
		...
		if err != nil {
			httputil.WriteError(w, http.StatusBadRequest, "INVALID_PARAMS", "want /gears/firmware/{depot}/{channel}")
			return
		}
...
func (s *Server) writeGearRegistrations(w http.ResponseWriter, items []gears.Gear) {
	...
	httputil.WriteJSON(w, http.StatusOK, map[string]any{"items": registrations})
}
...
func (s *Server) handleAdminGearResource(w http.ResponseWriter, r *http.Request, path string) {
	...
	case strings.HasSuffix(path, ":approve") && r.Method == http.MethodPost:
		...
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
			httputil.WriteError(w, http.StatusBadRequest, "INVALID_ROLE", err.Error())
			return
		}
	...
	case strings.HasSuffix(path, ":refresh") && r.Method == http.MethodPost:
		...
		if err != nil {
			...
			case errors.Is(err, ErrDeviceOffline):
				httputil.WriteError(w, http.StatusConflict, "DEVICE_OFFLINE", err.Error())
			case errors.Is(err, gears.ErrGearNotFound):
				httputil.WriteError(w, http.StatusNotFound, "GEAR_NOT_FOUND", err.Error())
			default:
				httputil.WriteError(w, http.StatusBadGateway, "DEVICE_REFRESH_FAILED", err.Error())
			}
			return
		}
	...
	case strings.HasSuffix(path, "/runtime") && r.Method == http.MethodGet:
		...
		httputil.WriteJSON(w, http.StatusOK, s.peerRuntime(publicKey))
	...
	case r.Method == http.MethodGet:
		...
		httputil.WriteJSON(w, http.StatusOK, gear.Registration())
	default:
		httputil.WriteError(w, http.StatusBadRequest, "INVALID_PARAMS", "unsupported route")
	}
}
